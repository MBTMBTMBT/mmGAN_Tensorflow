...
class BrainMRIData(Dataset):
    def __init__(self, h5path, mean_var_path, group_name, dataset_name,
                 load_seg,
        ...
        if group_name is None or \
                group_name not in self.valid_parent_names:
        ...
        self.h5path = h5path
        self.h5file = h5py.File(h5path, 'r', swmr=True)
        ...
        try:
            ...
            self.xdataset = self.h5file[self.group_name][self.xdataset_name]
            self.x_max = []
            ...
            if 'validation_data' not in self.xdataset_name and load_seg:
                self.ydataset = self.h5file[self.group_name][self.ydataset_name]
            self.name_dataset = self.h5file[self.group_name][self.name_dataset_name]
        ...
        self.load_pat_names = False
        self.load_seg = load_seg
        self.transform = transform

        if self.apply_normalization:
            # dataset will only be BRAST2018
            ...
            if which_normalization is None:
                ...
                logger.info('Using TANH normalization')
    ...
    def __len__(self):
        try:
            return len(self.h5file[self.group_name][self.xdataset_name])
        except ValueError:
            return 0
    ...
    @staticmethod
    def apply_normalization_for_ISLES(im, mean_var_file):
        """
    ...
    def reconnect(self):
        try:
            self.h5file = h5py.File(self.h5path, 'r', swmr=True)
            return True
        except:
            raise
...
def show_intermediate_results_BRATS(G, test_patient, save_path, all_scenarios, epoch, curr_scenario_range=None,
                                    batch_size_to_test=5, cuda=False):
        ...
        for curr_scenario in scenarios:
            ...
            for _num, batch_idx in enumerate(batch_indices):
                ...
                if cuda:
                    ...
                    x_test_r = patient['image'][batch_idx:batch_idx + batch_size, ...]
                    x_test_z = x_test_r.clone().type(torch.cuda.FloatTensor)
...
def show_intermediate_results(G, test_patient, save_path, all_scenarios, epoch, curr_scenario_range=None,
                              batch_size_to_test=5, seq_type='T1', dataset='ISLES2015', cuda=False):
    if isinstance(test_patient, list):
        patients = test_patient
    else:
        patients = [test_patient]
...
def create_dataloaders(h5file_dir=os.path.join(TOP_LEVEL_PATH, 'out'),
                       h5file_name=PREPROCESSED_FILE_NAME,
                       parent_name='preprocessed',
                       dataset_name='training_data',
                       dataset_type='HGG',
                       mean_var_name=MEAN_VAR_NAME,
                       # I didn't prepare the names anyway, no I would not consider this now
                       # load_pat_names=True,
                       # Only consider the cropped type
    ...
    if transform_fn is None:
        transform_fn = [Resize(size=SPATIAL_SIZE_FOR_PREPROCESSING), ToTensor()]
    logger.info("Setting paths")
    # train_h5path = os.path.join(parent_path, 'BRATS_Combined.h5')

    # the dataset is always BRATS2018
    # if dataset == 'BRATS2018':

    # only consider the cropped data
    # if dataset_type == 'cropped':
    # train_h5path = os.path.join(parent_path, 'BRATS2018_Cropped.h5')
    # train_h5path = "BRATS2018/HDF5_Datasets/BRATS2018_Cropped.h5"
    # else:
    #     train_h5path = os.path.join(parent_path, 'BRATS2018.h5')

    train_h5path = os.path.join(h5file_dir, h5file_name + '_' + dataset_type + '.h5')
    mean_var_path = os.path.join(h5file_dir, dataset_type + '_' + mean_var_name)

    # if 'LGG' in dataset_type:
    #     mean_var_path = os.path.join(h5file_dir, mean_var_name + '_' + dataset_type)
        # mean_var_path = "BRATS2018/HDF5_Datasets/training_data_lgg_mean_std.p"
    # else:
    #     mean_var_path = os.path.join(h5file_dir, mean_var_name)
        # mean_var_path = "BRATS2018/HDF5_Datasets/training_data_hgg_mean_std.p"
    ...
    logger.info('Using the following paths:')
    logger.info('train_h5_path: {}'.format(train_h5path))
    logger.info('mean_var_path: {}'.format(mean_var_path))
    ...
    loader = BrainMRIData(train_h5path, mean_var_path, parent_name,
                          dataset_name, load_seg, transform, apply_normalization=apply_normalization,
                          which_normalization=which_normalization, dataset=dataset,
                          load_indices=load_indices, train_range=train_range)
    ...
    dataloader = DataLoader(loader, batch_size=1, shuffle=shuffle, num_workers=num_workers,
...
def impute_reals_into_fake(x_z, fake_x, label_scenario, cuda=False):
    for idx, k in enumerate(label_scenario):
        if k == 1:  # THIS IS A REAL AVAILABLE SEQUENCE
            fake_x[:, idx, ...] = x_z[:, idx, ...].clone().cuda() if cuda else x_z[:, idx, ...].clone()
...
def load_checkpoint(model, optimizer, filename, pickle_module, device):
    if os.path.isfile(filename):
        logger.info("Loading checkpoint '{}'".format(filename))
        checkpoint = torch.load(filename, pickle_module=pickle_module, map_location=device)
...
def calculate_metrics(G, patient_list,
                      ...
                      save_stats=False,
                      mean_var_file=None,
                      use_pytorch_ssim=False, seq_type='T1', cuda=True, save_img=SAVE_IMG):
    """
    For ISLES2015
    ...
    for (pat_ind, patient) in enumerate(patients):
        ...
        for curr_scenario in scenarios:
            ...
            for _num, batch_idx in enumerate(batch_indices):
                ...
                # save pic
                if _num == SAVE_IMG_SLICE // BATCH_SIZE and save_img:
                    for channel in range(4):
                        pic_path = "test_patient_%d_channel_%d_slice%d__epoch_%d.png" % (pat_ind, channel, SAVE_IMG_SLICE, epoch)
...
def calculate_metrics_pgan(G, patient_list,
                           ...
                           convert_normalization=False,
                           mean_var_file=None,
                           use_pytorch_ssim=False, seq_type='T1', cuda=False):
    """
    For ISLES2015
    all_scenarios: scenarios
    curr_scenario_range: None
    ...
    for (pat_ind, patient) in enumerate(patients):
        ...
        for _num, batch_idx in enumerate(batch_indices):
            if cuda:
                ...
                x_test_r = patient_image[batch_idx:batch_idx + batch_size, SEQ_IDX_SYNTH, ...].unsqueeze(1)
                x_test_z = patient_image[batch_idx:batch_idx + batch_size, SEQ_IDX, ...].unsqueeze(1)
...
def printTable(result_dict):
    logger.info("{:<10} {:<10} {:<10} {:<10}".format('Scenario', 'MSE', 'PSNR', 'SSIM'))
    result_keys = sorted(result_dict.keys())
    for k, v in zip(result_keys, result_dict.values()):
        logger.info("{:<10} {:<10.4f} {:<10.4f} {:<10.4f}".format(k, result_dict[k]['mse'],
                                                                  result_dict[k]['psnr'],
                                                                  result_dict[k]['ssim']))
