...
from tensorflow import GradientTape, float32, constant, cast, tensor_scatter_nd_update, summary, concat
...
def mse(slice_a: np.ndarray, slice_b: np.ndarray) -> float:
    return (np.square(np.subtract(slice_a, slice_b))).mean().item()


def psnr_np(pred, gt) -> float:
    # normalize images between [0, 1]
    epsilon = 0.00001
    epsilon2 = np.array(0.01, dtype=np.float32)
    ...
    if mse.item() == 0.0:
        psnr = 20 * np.log10(PIXEL_MAX / epsilon2)
    else:
...
def train(session_name: str, output_dir: str, tfrecords_train: list, tfrecords_val: list,
          ...
          epochs_per_dom_epoch: int, implicit_conditioning=True, curriculum_learning=True, focus_on_worst=True,
          debug=False):
    ...
    checkpoint = tf.train.Checkpoint(generator_optimizer=optimizer_g, discriminator_optimizer=optimizer_d,
                                     generator=generator, discriminator=discriminator)
    checkpoint_manager = tf.train.CheckpointManager(
        checkpoint, directory=checkpoint_dir, max_to_keep=epochs)
    ...
    for epoch_idx in range(start_epoch, epochs):
        ...
        dataset_train = get_dataset(tfrecords=tfrecords_train, compression_type='ZLIB', batch_size=batch_size_train,
                                    drop_remainder=True,
        ...
        for sub_epoch_idx in range(sub_epochs):
            ...
            for batch_idx in range(batches_per_sub_epoch):
                ...
                for each_key in batch_summary.keys():
                    batch_summary[each_key] = batch_summary[each_key].numpy(
                    ).item()
            ...
            val_summary, bad_scenario = validate(val_iter=val_iter, val_logger=val_logger, generator=generator,
                                                 scenarios=scenarios, json_dir=json_dir, epoch=epoch_idx,
                                                 sub_epoch=sub_epoch_idx,
    ...
    with GradientTape() as g_tape, GradientTape() as d_tape:
        ...
        '''if debug:
            ...
            for idx, label in enumerate(label_scenarios):
                ...
                for i in range(4):
                    plt.subplot(1, 4, i + 1)
                    plt.imshow(label_list_fake[idx][i])
                    label_title = label_str + channels[i] + "label_fake"
            ...
            for num_slice, label_scenario in enumerate(label_scenarios):
                for idx_curr_label, i in enumerate(label_scenario):
                    if i == 0:
                        ...
                        loss_pixel += mae(fake_x[num_slice, idx_curr_label, ...],
                                          batch[num_slice, idx_curr_label, ...])
        ...
        g_train_total_loss = (constant(1, dtype=float32) -
                              lambda_param) * loss_gan + lambda_param * loss_pixel
        # g_train_total_loss = loss_pixel

        # total discriminator loss
        d_train_total_loss = constant(
            0.5, dtype=float32) * (loss_fake + loss_real)

    # apply gradients
    gradients_of_generator = g_tape.gradient(
        g_train_total_loss, generator.trainable_variables)
    gradients_of_discriminator = d_tape.gradient(
        d_train_total_loss, discriminator.trainable_variables)
...
def validate(val_iter: iter, val_logger: logging.Logger, generator, scenarios: list,
    ...
    for idx in range(batch_num):
        ...
        for each_scenario in scenarios:
            ...
            for c, each_channel in enumerate(each_scenario):
                if each_channel == 0:
                    fake_channel_arr = np.expand_dims(fake_img[0, c, ...], 2)
                    real_channel_arr = np.expand_dims(batch[0, c, ...], 2)
                    ...
                    metrics[sc_idx]['mse'].append(
                        ...
                        mse(fake_channel_arr / (np.max(fake_channel_arr) + 0.0001),
                            real_channel_arr / (np.max(real_channel_arr) + 0.0001)))
                    metrics[sc_idx]['psnr'].append(
                        psnr_np(fake_channel_arr, real_channel_arr))
                    metrics[sc_idx]['ssim'].append(
                        tf.image.ssim(fake_channel_arr / (np.max(fake_channel_arr) + 0.0001),
                                      real_channel_arr / (np.max(real_channel_arr) + 0.0001), max_val,
                                      filter_size=ssim_filter_size).numpy().item())
            sc_idx += 1
...
if __name__ == '__main__':
    ...
    train(session_name="comb0_tf", output_dir=r"E:\my_files\programmes\python\mri_gan_output",
